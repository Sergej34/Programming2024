package Classwork_36.linked_list;
...
//                }
    ...
    public int lastIndexOf(Object o) {
        int index = 0; // отслеживает текущий индекс
        int lastIndex = -1; // отслеживает последний найденный индекс

        if (o != null) {
            // Проходим по всему списку
            for (Node<E> node = first; node != null; node = node.next, index++) {
                // Если нашли совпадение, сохраняем индекс
                if (o.equals(node.data)) {
                    lastIndex = index;
                }
            }
        } else {
            // Для случая, когда объект o равен null
            for (Node<E> node = first; node != null; node = node.next, index++) {
                // Если нашли null, сохраняем индекс
                if (node.data == null) {
                    lastIndex = index;
                }
            }
        ...
        // Возвращаем последний найденный индекс (или -1, если объект не найден)
        return lastIndex;
    }
